...
//#define CNTR_CPU "\\Process(FileSeek)\\% Processor Time" // % of cpu in use
...
//fmt
#define CNTR_PROCESS_HANDLECOUNT_s		"\\Process(%s)\\Handle Count"
#define CNTR_PROCESS_PROCESSORTIME_s	"\\Process(%s)\\%%Processor Time"
#define CNTR_PROCESS_THREADCOUNT_s		"\\Process(%s)\\Thread Count"

#define CNTR_THREAD_CPU_s_d				"\\Thread(%s/%d)\\%% Processor Time"
#define CNTR_THREAD_PRIVILEGEDTIME_s_d	"\\Thread(%s/%d)\\%% Privileged Time"
#define CNTR_THREAD_UserTIME_s_d		"\\Thread(%s/%d)\\%% User Time"
#define CNTR_THREAD_ContextSwitches_s_d "\\Thread(%s/%d)\\Context Switches/sec"
#define CNTR_THREAD_ID_s_d				"\\Thread(%s/%d)\\ID Thread"
...
protected:
	...
	std::vector<PPDHCOUNTERSTRUCT> m_aCounters; // the current counters
	int m_nNextIndex;
};
