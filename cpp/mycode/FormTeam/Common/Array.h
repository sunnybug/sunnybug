...
public:
	Array()											{ m_set.resize(SZ,T()); }
	explicit Array(int nSize, T data)				{ m_set.resize(nSize, data); }
	Array(const Array& obj)							{ m_set = obj.m_set; }
	Array<T>& operator=(const Array<T>& obj)		{ if(this==&obj) return *this; m_set = obj.m_set; return *this; }
...
public:
	...
	bool	Find(const T& obj)						{ return find(m_set.begin(), m_set.end(), obj) != m_set.end(); }
	...
	template<TYPENAME X>
		void CopyFrom(const Array<X>& obj)				{ Clear(); for(int i=0;i<obj.Size();i++) Push(obj[i]); }
	template<TYPENAME X>
