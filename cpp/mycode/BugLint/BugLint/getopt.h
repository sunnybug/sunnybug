...
private:
	...
	static const int maxpnamelength;
...
//	std::map<std::string, ParamContainer> tlist;
	...
	struct cmdlineel {
		...
		std::string val;
		int pos;
		cmdlineel(elflag _flag, int _pos, std::string _val=""):flag(_flag),pos(_pos),val(_val) {}
	};
...
public:
	...
	************/
	...
	static void setMsgList(char **_messages) {messages=_messages;}
	...
	*/
	...
	bool is_set(std::string pname) const
	{
		if(plist.find(pname)==plist.end()) return false;
		return plist.find(pname)->second.wasset;
	}
	/*
	...
	*/
	...
	void dumpHelp(std::ostream& strm, bool showparamlist=true, unsigned int width=0, std::string subtopic="") const;
	/*
	...
	*/ 
	...
	static void printHelpTopic(std::ostream& strm, std::string topic, int indent, int width, bool linebreak=true);
