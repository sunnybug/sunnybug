...
class CCfg:
	def __init__(self,  ini_file,  history_file,  app_dir):
		...
		try:
			...
			self.xml_history =ET.ElementTree(ET.Element("history"))
	...
	def GetCheckMiniute(self):
		if self.xml_history_log.get('check_minute') == None:
			self.xml_history_log.set('check_minute',  '60')
	...
	def GetDownSpeed(self):
		if self.xml_history_log.get('down_speed') == None:
			self.xml_history_log.set('down_speed',  '500')
